            ...
            >
              ...
              {navLinks.map((link) => (
                <button key={link.href} onClick={() => handleNav(link.href)}
                  className="px-3 py-1.5 text-sm font-body text-white/55 hover:text-white rounded-xl transition-all duration-200 hover:bg-white/5 whitespace-nowrap">
                  {link.label}
                </button>
              ))}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
            ...
            {navLinks.map((link) => (
              <button key={link.href} onClick={() => handleNav(link.href)}
                className="text-left px-4 py-3 rounded-xl text-white/65 hover:text-white hover:bg-white/5 transition-all font-body">
                {link.label}
              </button>
            ))}
